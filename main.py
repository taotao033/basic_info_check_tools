...
class App_login(QWidget):
    ...
    def init_UI_login(self):
        ...
        self.db_col_name_text_b.setText(self.col_temp1)
    ...
    def button_login_on_click(self):
        ...
        self.col_temp1 = self.db_col_name_text_b.text()
        if not self.col_temp1:
        ...
        if connect_result:
            print("连接成功！ 进入主界面...")
            if self.job_num and self.mongodb_ip and self.port and self.mongodb_name and self.col_temp1 \
                ...
                main_app.col_temp1_ = self.col_temp1
...
# check_status_flag = "0"
# print("当前进行的是： 基本信息表 第" + str(int(check_status_flag) + 1) + "次校对...")
...
class App(QWidget):  # 继承自 QWidget类
    def __init__(self, job_num="", mongodb_ip="192.168.1.23", port="27017", mongodb_name="information",
        ...
        self.find_one_data_in_temp1 = {}
        self.person_id = ""
        self.person_id_prcessed_list = []  # 用于存储col_temp1中已经校对完的person_id
        ...
        self.col_temp1_ = col_temp1
        self.col_temp2_ = col_temp2
        self.col_final_ = col_final
    ...
    def initUI_main(self):
        ...
        self.query_data()
        self.create_table(self.find_one_data_in_temp1)
        ...
        # self.button4 = QPushButton('上一页修改保存', self)
        # self.button4.setToolTip("当需要重新修改上一页数据时，修改完成后请点击此按钮保存修改")
        # self.button4.setFixedSize(90, 30)
        ...
        """
            自定义人物检索按钮
        """
        self.ser_person_info_button = QPushButton('自定义人物检索', self)
        self.ser_person_info_button.setToolTip("根据检索条件，查找人物信息")
        self.ser_person_info_button.setFixedSize(100, 30)
    ...
    def query_data(self):
        ...
        if self.find_one_data_in_temp1:
            self.person_id = self.find_one_data_in_temp1["person_id"]
            self.information_ancient_base_temp1.update_one({"person_id": self.person_id}, {"$set": {"check_status": "-1"}})
            print("初版库（AI）####" + self.col_temp1_ + "####person_id: " + self.person_id + " "
                  + '\"' + "check_status" + '\"' + "更新为: -1")
        else:
            print("Congratulations! 初版库（AI）####" + self.col_temp1_ + "#######数据校对完毕。您辛苦了！")
        ...
        if self.page_up_flag:  # 当上一页被点击后执行
            ...
            if self.FLAG_ALL_RIGHT:
                ...
                if self.index != len(self.person_id_prcessed_current_list):
                    ...
                    count_row_ = 0
                    for k, v in self.find_one_data_in_temp1.items():
                        self.table.setItem(count_row_, 0, QTableWidgetItem(str(k)))
                        self.table.setItem(count_row_, 1, QTableWidgetItem(str(v)))
                        count_row_ += 1
    ...
    def button3_on_click(self):
        ...
        if buttonReply1 == QMessageBox.Yes:
                ...
                if self.FLAG_ALL_RIGHT:
                    ...
                    if temp_dict["person_id"] not in self.person_id_prcessed_current_list:
                        self.information_ancient_base_temp1.update_one({"person_id": self.person_id},
                                                                       {"$set": {"check_status":
                                                                                 str(int(self.check_status_) + 1)}})
                        print("退出时，初版库（AI）####" + self.col_temp1_ + "####person_id: " + temp_dict["person_id"] + " "
                              + '\"' + "check_status" + '\"' + "更新为: " + str(int(self.check_status_) + 1))
            ...
            print("退出时，没有保存当前数据，初版库（AI）####" + self.col_temp1_ +
